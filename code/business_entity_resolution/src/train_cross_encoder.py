...
def prepare_cross_encoder_data(
    ...
    # P5 fix: Build separate name/addr/city lookups for semantic tagging
    ...
    for s1, s2, lbl in zip(df_selected["s1_id"], df_selected["s2s3_id"], df_selected["label"]):
        ...
        if n1 or a1:
            # P5 fix: Inject semantic boundaries before tokenization
            t1 = f"[NAME] {n1} [ADDR] {a1} [CITY] {c1}"
            t2 = f"[NAME] {n2} [ADDR] {a2} [CITY] {c2}"
