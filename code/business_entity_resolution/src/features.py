...
FEATURE_NAMES = [
    ...
    "addr_missing_either",
    "addr_missing_both",
    # Component features (7) — includes P2 city/state geo features
    "street_number_match",
    "postal_code_match",
    "numeric_overlap",
    "postal_code_edit_dist",
    "country_match",
    "city_exact_match",       # P2 fix: prevents cross-city false merges
    "city_jaro_winkler",      # P2 fix: fuzzy city name comparison
    # Combined features (5)
...
def extract_pair_features(
    ...
    # P2 fix: city matching features to prevent cross-city false merges
    ct1 = _safe_str(city_s1)
    ct2 = _safe_str(city_s2)
    if ct1 and ct2:
        feats[30] = 1.0 if ct1 == ct2 else 0.0
        feats[31] = JaroWinkler.normalized_similarity(ct1, ct2)
    else:
        feats[30] = -0.5  # missing data sentinel
        feats[31] = -0.5

    # ── Combined features ───────────────────────────────
    if na1 and na2:
        feats[32] = fuzz.token_sort_ratio(na1, na2) / 100.0
        feats[33] = JaroWinkler.normalized_similarity(na1, na2)

    feats[34] = _idf_weighted_overlap(n1, n2, idf_name) if n1 and n2 else 0.0
    feats[35] = _idf_weighted_overlap(a1, a2, idf_addr) if a1 and a2 else 0.0
    feats[36] = _idf_weighted_overlap(na1, na2, idf_combined) if na1 and na2 else 0.0

    # ── Embedding cosine ────────────────────────────────
    feats[37] = emb_cosine

    # ── Meta features ───────────────────────────────────
    feats[38] = 1.0 if entity_id_s2.startswith("S2-") else 0.0  # source indicator
    feats[39] = min(len(n1), len(n2)) / max(len(n1), len(n2), 1)  # name length ratio
    feats[40] = min(len(a1), len(a2)) / max(len(a1), len(a2), 1) if (a1_present and a2_present) else 0.0
    feats[41] = min(len(na1), len(na2)) / max(len(na1), len(na2), 1)
...
def extract_features_for_pairs(
    ...
    s1_cols = {
        ...
        'city': df_s1["city"].fillna("").astype(str).to_numpy(dtype=object),  # P2 fix
    }
    t_cols = {
        ...
        'city': df_targets["city"].fillna("").astype(str).to_numpy(dtype=object),  # P2 fix
    }
    ...
    def _flush_batch(start, end, c_idx):
        ...
        chunk_df = pd.DataFrame({
            ...
            "pc1": s1_cols['pc'][idx1],
            "c1": s1_cols['c'][idx1],
            "city1": s1_cols['city'][idx1],  # P2 fix
            "n2": t_cols['n'][idx2],
            "a2": t_cols['a'][idx2],
            "na2": t_cols['na'][idx2],
            "sn2": t_cols['sn'][idx2],
