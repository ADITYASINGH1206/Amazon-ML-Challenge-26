...
def extract_features_for_pairs(
    ...
    pair_count = 0
    for s1_eid, cand_set in tqdm(candidates.items(), desc="Feature extraction",
                                  mininterval=10):
        ...
        for cand_eid in cand_set:
            ...
            if len(batch_features) >= config.FEATURE_BATCH_SIZE:
                ...
                batch_s1_ids, batch_s2s3_ids, batch_features = [], [], []
    ...
    df_features = pd.concat(results, ignore_index=True) if results else pd.DataFrame()
...
def load_embeddings_for_features(split: str, df_s1: pd.DataFrame,
    ...
    if not target_cache.exists() or not query_cache.exists():
        ...
        emb_targets = compute_embeddings(t_texts, save_path=target_cache)
        emb_s1 = compute_embeddings(q_texts, save_path=query_cache)
    else:
