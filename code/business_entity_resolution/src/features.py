...
FEATURE_NAMES = [
    ...
    "country_match",
    "city_exact_match",       # Prevents cross-city false merges
    "city_jaro_winkler",      # Fuzzy city name comparison
    # Combined features (5)
...
def extract_features_for_pairs(
    ...
    s1_cols = {
        ...
        'city': df_s1["city"].fillna("").astype(str).to_numpy(dtype=object) if "city" in df_s1.columns else np.array([""] * len(df_s1), dtype=object),
    }
    t_cols = {
        ...
        'city': df_targets["city"].fillna("").astype(str).to_numpy(dtype=object) if "city" in df_targets.columns else np.array([""] * len(df_targets), dtype=object),
    }
